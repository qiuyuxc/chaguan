...
积分商城(勋章 / 签到加成 / 自定义商品)、积分流水可对账。

**抽奖帖**:回复即参与,可设参与投入积分、参与人数上限(先来后到)、定点自动开奖。
分两类 —— **实物奖**平台只随机抽人、出中奖名单,奖品楼主自己发;**积分奖**由楼主发帖时
预扣一笔奖池(可与参与者投入合流),开奖时按随机切点法拆开自动打给中奖者,每人至少 1 积分,
所以中奖人数不会超过奖池总额。开不了奖(无人参与)或帖子被删,奖池与投入都原路退回。
...
BASE=http://localhost:8090 bash scripts/smoke.sh      # 496 条 curl 断言

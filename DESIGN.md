...
# Forum Extension (bbs 项目适配)

本项目是一个 Go + SQLite 的讨论论坛,不是 Linear 的营销站。以下规则覆盖/补充上文,冲突时以本节为准。

## 字体(中文优先)

Linear 的自定义字体不可用,且本项目主要承载中文内容。使用系统字体栈,不加载任何 webfont:

```css
--font-sans: Inter, -apple-system, "PingFang SC", "HarmonyOS Sans SC", "MiSans", "Microsoft YaHei", "Noto Sans SC", sans-serif;
--font-mono: "JetBrains Mono", ui-monospace, SF Mono, Menlo, monospace;
```

- 负字距(letter-spacing)规则只作用于拉丁字符标题;中文标题 letter-spacing 一律为 0(CJK 没有负字距排版习惯)。
- font-weight 体系沿用:标题 600、正文 400、按钮/标签 500。中文回退字体在移动端都支持这些字重。

## 组件映射(论坛 UI → Linear tokens)

| 论坛组件 | 基于 | 规范 |
|---|---|---|
| `thread-row`(主题行) | `changelog-row` | 透明背景,1px `{colors.hairline}` 底部分隔,垂直 padding 12–16px;标题 `{typography.body}` 600,meta 信息 `{typography.caption}` `{colors.ink-subtle}` |
| `post-card`(帖子卡) | `feature-card` | `{colors.surface-1}` 背景,`{rounded.lg}` 12px 圆角,1px `{colors.hairline}` 边框,内边距 16px;头像圆形 36px `{colors.surface-3}` 底 + 首字母 |
| `nav-badge`(未读数) | `status-badge` | pill,`{colors.surface-2}` 背景 `{colors.ink-muted}` 文字;有未读时改 `{colors.primary}` 背景 |
| `cat-tab`(版块切换) | `pricing-tab-*` | pill,选中 = `{colors.surface-2}` 抬升;触控目标 ≥44px |
| `btn-primary`("发新帖"/"回复") | `button-primary` | `{colors.primary}`,8px 圆角,padding 8px 14px |
| `text-input / textarea`(编辑器) | `text-input` | `{colors.surface-1}` 背景,`{colors.hairline-strong}` 边框,聚焦 2px `{colors.primary-focus}` 50% 透明度外圈 |
| `pagination`(分页) | `pricing-tab-*` | pill 页码,当前页 = surface-2 抬升 |
| `role-badge`(管理员/版主) | `status-badge` | pill,admin 用 `{colors.primary}`,mod 用 `{colors.ink-tertiary}` |
| `top-nav` | `top-nav` | 56px,`{colors.canvas}`,底部 1px hairline,sticky |

## 布局

- 内容最大宽度 880px 居中(论坛是阅读流,不用 1280px 营销宽度);左右 padding:桌面 24px,移动 16px。
- 移动端(主场景):帖子卡全宽;主题行两行布局(标题一行,meta 一行);导航只保留站名 + 用户/登录,无需汉堡菜单。
- 触控目标一律 ≥44px。

## 红线(覆盖原营销规则)

- ~~"每个 section 放产品截图"~~ —— 不适用,论坛没有产品截图。
- ~~"Don't ship a light-mode page"~~ —— 沿用:本项目只有深色模式,不做 light mode 开关。
- 沿用:薰衣草蓝只用于 CTA、焦点环、链接强调、admin 徽章、未读角标 —— 绝不做大面积背景。
- 沿用:层级靠 surface 阶梯 + hairline,不用阴影。
- 沿用:不引入第二种彩色。
